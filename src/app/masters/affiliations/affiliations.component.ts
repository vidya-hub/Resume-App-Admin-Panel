...
export class AffiliationsComponent implements OnInit {
  ...
  fnGetCategories() {
    ...
    this.service.post('admin/get_all_affiliations', form).then((result: any) => {
      this.page++;
  ...
  fnSubmitCategory() {
    ...
    let form = new HttpParams().set('title', this.name).set("expFlag",JSON.stringify(this.selectedExp)).set('jobtitleId',this.jobtitleId)
    // form.append('center_id', this.center_id)
    // form.append('type', this.type)
    this.pleaseWaitBtn = true;
    this.service.post('admin/add_affiliations', form).then((result: any) => {
  ...
  fnUpdateCategory() {
    ...
    let form = new HttpParams().set('title', this.name).set('id', this.selectedCategory._id).set('jobtitleId',this.jobtitleId).set("expFlag",JSON.stringify(this.selectedExp))
    this.pleaseWaitBtn = true;
    this.service.post('admin/update_affiliations', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnEnableCategory() {
    ...
    this.pleaseWaitBtn = true;
    this.service.post('admin/enable_affiliations', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnDisableCategory() {
    let form = new HttpParams().set('id', this.selectedCategory._id)
    this.pleaseWaitBtn = true;
    this.service.post('admin/disable_affiliations', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnDeleteCategory() {
    let form = new HttpParams().set('id', this.selectedCategory._id)
    this.pleaseWaitBtn = true;
    this.service.post('admin/delete_affiliations', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
