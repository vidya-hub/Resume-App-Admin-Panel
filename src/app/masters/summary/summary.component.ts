...
export class SummaryComponent implements OnInit {
  ...
  fnGetCategories() {
    ...
    this.service.post('admin/get_all_summary', form).then((result: any) => {
      this.page++;
  ...
  fnSubmitCategory() {
    ...
    let form = new HttpParams().set("expFlag",JSON.stringify(this.selectedExp)).set('title', this.name).set('jobtitleId',this.jobtitleId)
    // form.append('center_id', this.center_id)
    // form.append('type', this.type)
    this.pleaseWaitBtn = true;
    this.service.post('admin/add_summary', form).then((result: any) => {
  ...
  fnUpdateCategory() {
    ...
    let form = new HttpParams().set("expFlag",JSON.stringify(this.selectedExp)).set('title', this.name).set('id', this.selectedCategory._id).set('jobtitleId',this.jobtitleId)
    this.pleaseWaitBtn = true;
    this.service.post('admin/update_summary', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnEnableCategory() {
    ...
    this.pleaseWaitBtn = true;
    this.service.post('admin/enable_summary', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnDisableCategory() {
    let form = new HttpParams().set('id', this.selectedCategory._id)
    this.pleaseWaitBtn = true;
    this.service.post('admin/disable_summary', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
  ...
  fnDeleteCategory() {
    let form = new HttpParams().set('id', this.selectedCategory._id)
    this.pleaseWaitBtn = true;
    this.service.post('admin/delete_summary', form).then((result: any) => {
      if (result && result.msg && result.msg != '') {
        this.toast.success(result.msg)
      }
